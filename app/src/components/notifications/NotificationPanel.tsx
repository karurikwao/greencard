...
import { Bell, Check, ChevronDown, ChevronRight, ExternalLink, MessageSquare, CreditCard, Gift, Trophy, Info } from 'lucide-react';
...
export function NotificationPanel({ className }: NotificationPanelProps) {
  ...
  const handleOpenNotification = async (notification: UserNotification) => {
    setSelectedNotificationId(prev => prev === notification.id ? null : notification.id);
                      ...
                      {isExpanded && (
                        <div className="border-t border-slate-200/80 bg-white/90 p-4">
                          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                            <RichMessageContent content={notification.message} className="text-slate-800" />
                          </div>
