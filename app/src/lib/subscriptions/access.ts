...
export function buildEffectiveSubscription(
  ...
  // Can renew if expired/canceled/inactive
  const canRenew = effectiveStatus === 'expired' || 
    effectiveStatus === 'inactive' ||
    (effectiveStatus === 'canceled' && hasAccess === false);
  
  // Can upgrade if trialing, active, or in grace period
  const canUpgrade = ['trialing', 'active', 'past_due', 'grace_period'].includes(effectiveStatus) ||
    (effectiveStatus === 'canceled' && hasAccess);
...
export function checkFeatureAccess(
  ...
  if (!subscription) {
    ...
    return {
      ...
      reason: trialFeatures[feature] ? undefined : 'This feature requires a premium plan',
      requiresUpgrade: !trialFeatures[feature],
      currentPlan: 'trial',
      effectiveStatus: 'inactive',
    };
  }
...
/**
 * Check if user has premium access (any paid plan with active access)
 */
export function hasPremiumAccess(
  ...
  return effectiveSub.hasAccess && 
    ['monthly', 'lifetime', 'interviewPass'].includes(effectiveSub.planType);
}
