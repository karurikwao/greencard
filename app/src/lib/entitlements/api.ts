...
/**
* Trial entitlements - for authenticated users with explicit trial started
*
* This requires a subscription row in the database with plan_type = 'trial'
*/
/**
* Trial entitlements - for when user explicitly starts a trial
...
allowed: false,
requiresUpgrade: true,
reason: 'Sign up to start your free trial'
},
pdfDownloads: {
allowed: false,
requiresUpgrade: true,
...
allowed: false,
requiresUpgrade: true,
reason: 'Couple Compare requires a premium plan'
},
readinessCheck: {
allowed: true,
...
if (!hasAccess) {
return {
aiInterview: { allowed: false, requiresUpgrade: true, reason: 'Subscription required' },
pdfDownloads: { allowed: false, requiresUpgrade: true, reason: 'Subscription required' },
coupleCompare: { allowed: false, requiresUpgrade: true, reason: 'Subscription required' },
readinessCheck: { allowed: false, requiresUpgrade: true, reason: 'Subscription required' },
progressTracking: { allowed: true, requiresUpgrade: false, level: 'basic' },
};
}

const isPaid = ['monthly', 'lifetime', 'interviewPass'].includes(planType);

return {
aiInterview: {
allowed: true,
requiresUpgrade: false,
level: isPaid ? 'full' : 'limited',
},
pdfDownloads: {
allowed: isPaid,
requiresUpgrade: !isPaid,
reason: isPaid ? undefined : 'PDF downloads require a premium plan',
},
coupleCompare: {
allowed: isPaid,
requiresUpgrade: !isPaid,
