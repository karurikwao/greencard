...
import { 
  ...
  upgradeSubscription as upgradeSubscriptionUtil,
  ...
  try {
    ...
    if (legacy) {
      ...
      const migrated: LegacyUserSubscription = {
        ...
        passEnd: null,
        pdfDownloadsLocked: parsed.pdfDownloadsLocked ?? true,
...
export function usePricing() {
  ...
  const [localSubscription, setLocalSubscription] = useLocalStorage<LegacyUserSubscription>(
    SUBSCRIPTION_STORAGE_KEY,
  ...
  // SECURITY: localStorage is NEVER used for premium access control
  // - If user is authenticated: entitlements from Supabase are authoritative
  // - If user is anonymous: default trial entitlements are used (no premium features)
  ...
  const featureAccess: FeatureAccess = useMemo(() => {
    ...
    if (entitlements) {
      const isPaid = entitlements.subscription.isPaid || entitlements.subscription.isLifetime;
      ...
      return {
        ...
        timelineBuilder: isPaid,
        readinessCheck: entitlements.features.readinessCheck.allowed,
        printablePacks: isPaid,
        futurePacks: isPaid,
      };
    }
    // While loading or if entitlements fail, default to trial (no premium features)
  ...
  const isPracticeLocked = useMemo(() => {
    ...
    return !effectiveSubscription.hasAccess;
  }, [entitlements, effectiveSubscription]);
  
  // Check if has premium access
  const hasPremium = useMemo(() => {
    if (entitlements) {
      return entitlements.subscription.hasAccess && entitlements.subscription.isPaid;
    }
  ...
  const upgradePlan = useCallback(async (plan: PlanType) => {
    // For now, use the local upgrade path (will be replaced with Stripe checkout)
    const updated = upgradeSubscriptionUtil(localSubscription, plan);
    setLocalSubscription(updated);
    
    // Refresh entitlements after upgrade (will pick up from server if user is authenticated)
    await refreshEntitlements();
  }, [localSubscription, setLocalSubscription, refreshEntitlements]);
