...
-- ============================================================================
-- InterviewReady Complete Fresh-Install SQL for Brand New Supabase Projects
-- Created: 2026-03-16 (Security Final v4)
-- 
-- CRITICAL: This is for NEW Supabase projects only.
-- Run this in the Supabase SQL Editor.
--
-- SECURITY NOTES:
-- - Content RPCs derive caller identity from auth.uid(), NOT caller-supplied params
-- - Admin/reporting functions are restricted to admin/service_role
-- - All SECURITY DEFINER functions have appropriate access controls
--
-- Current Architecture (NO legacy tables):
-- - user_profiles with non-recursive admin checks
-- - user_subscriptions with plan_config
-- - ai_daily_usage + ai_session_tracking (current AI system)
-- - pdf_assets with secure download tracking
-- - site content management (announcements, trust snippets, content blocks)
-- - content dismissals + interactions
-- - seo expansion system
-- - site_verification_codes (head/footer/body_end injection)
-- - refund/support/notification systems
-- - answer_example_candidates (admin review pipeline)
-- ============================================================================
...
-- REMOVED: uuid-ossp extension no longer needed (using gen_random_uuid() instead)
...
-- ============================================================================
-- SECTION 3: USER PROFILES AND ADMIN ROLES (NON-RECURSIVE)
-- ============================================================================
...
-- Non-recursive admin helper functions (SECURITY DEFINER to avoid recursion)
...
-- RLS Policies (non-recursive using helper functions)
...
-- ============================================================================
-- SECTION 4: USER SUBSCRIPTIONS (Complete with all payment-ready fields)
-- ============================================================================
...
CREATE POLICY "Anyone can view plan config" ON plan_config
  FOR SELECT TO anon, authenticated
  USING (is_active = true);

CREATE POLICY "Service role can manage plan config" ON plan_config
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ============================================================================
-- SECTION 7: AI USAGE TRACKING (Current Architecture)
-- ============================================================================
-- Uses ai_daily_usage + ai_session_tracking (NOT legacy ai_interviews)
...
-- NOTE: Does NOT reference last_activity_at (column does not exist in current schema)
...
BEGIN
  ...
  IF v_usage_record.sessions_count >= v_max_sessions THEN
    RETURN QUERY SELECT
      false,
      format('Daily session limit reached (%s per day)', v_max_sessions)::TEXT,
      v_plan_type, v_max_sessions, v_max_turns,
...
-- ============================================================================
-- SECTION 8: PDF ASSETS AND SECURE DOWNLOAD TRACKING
-- ============================================================================

-- Storage bucket (must run after storage schema exists)
DO $$
BEGIN
...
CREATE POLICY "Anyone can read pdf_assets metadata" ON pdf_assets
  FOR SELECT TO anon, authenticated
  USING (true);

CREATE POLICY "Only service role can modify pdf_assets" ON pdf_assets
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);

-- PDF Download Events (current status values)
...
-- ============================================================================
-- SECTION 9: SITE CONTENT MANAGEMENT
-- ============================================================================
...
-- RLS for content tables - RESTRICTED PUBLIC ACCESS
-- IMPORTANT: Direct table reads are restricted. Use RPC functions for filtered public reads.
...
CREATE POLICY "Service role full access on announcements" ON site_announcements
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Service role full access on trust snippets" ON site_trust_snippets
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Service role full access on content blocks" ON site_content_blocks
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ============================================================================
-- SECTION 10: CONTENT DISMISSALS AND INTERACTIONS
-- ============================================================================
...
-- ============================================================================
-- SECTION 11: CONTENT ACCESS FUNCTIONS (RPC-based - SAFE, NO SPOOFING)
-- ============================================================================
-- IMPORTANT: These functions derive caller identity from auth.uid().
-- They do NOT trust caller-supplied user_id or user_role.
--
-- Security model:
-- - User identity: derived from auth.uid()
-- - User role: derived from actual subscription status (trial/paid/expired)
-- - Dismissal filtering: uses auth.uid() directly, not caller-supplied value
-- - Anon users: treated as anonymous with no dismissal tracking

-- Helper to determine effective audience role from actual subscription state
CREATE OR REPLACE FUNCTION get_caller_audience_role()
RETURNS TEXT AS $$
DECLARE
  v_user_id UUID;
  v_sub RECORD;
BEGIN
  ...
  -- Get effective subscription for this user
  SELECT * INTO v_sub FROM get_effective_subscription(v_user_id);
  
  -- Determine role based on actual subscription state
...
-- FIXED: get_active_announcements no longer trusts caller-supplied role/id
...
DECLARE
  ...
  v_caller_role TEXT;
BEGIN
  -- Derive caller identity from auth, NOT from caller-supplied params
  v_caller_id := auth.uid();
  v_caller_role := get_caller_audience_role();
  
  RETURN QUERY
  SELECT 
    a.id, a.title, a.body, a.announcement_type, a.is_dismissible,
  ...
  WHERE a.status = 'published'
    ...
    AND (
      ...
      OR a.id NOT IN (
        SELECT cd.content_id FROM content_dismissals cd
        WHERE cd.user_id = v_caller_id AND cd.content_type = 'announcement'
      )
    )
...
-- FIXED: get_active_trust_snippets no longer trusts caller-supplied role/id
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    s.id, s.title, s.subtitle, s.icon_name, s.cta_text, s.cta_link, s.priority
  ...
  WHERE s.status = 'published'
    ...
    AND (
      ...
      OR s.id NOT IN (
        SELECT cd.content_id FROM content_dismissals cd
        WHERE cd.user_id = v_caller_id AND cd.content_type = 'trust_snippet'
      )
    )
...
-- FIXED: get_active_content_blocks no longer trusts caller-supplied role/id
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    b.id, b.title, b.body, b.block_type, b.group_key, b.group_order,
  ...
  WHERE b.status = 'published'
    ...
    AND (
      ...
      OR b.id NOT IN (
        SELECT cd.content_id FROM content_dismissals cd
        WHERE cd.user_id = v_caller_id AND cd.content_type = 'content_block'
      )
    )
...
-- ============================================================================
-- SECTION 12: SEO SETTINGS AND EXPANSION SYSTEM
-- ============================================================================
...
-- SEO Expansion Rebuild Attempts
...
-- FIXED: update_sitemap_frequency requires admin
...
-- FIXED: update_seo_expansion_page_status requires admin
...
-- FIXED: record_scheduler_run requires admin
...
-- FIXED: record_rebuild_attempt requires admin
...
BEGIN
  RETURN QUERY
  SELECT 
    sera.id, sera.triggered_by, sera.triggered_at, sera.status, sera.reason,
...
CREATE POLICY "Anyone can read enabled verification codes" ON site_verification_codes
  FOR SELECT
  USING (is_enabled = true);

CREATE POLICY "Only admins can manage verification codes" ON site_verification_codes
  FOR ALL TO authenticated
  USING (is_admin())
  WITH CHECK (is_admin());

CREATE POLICY "Service role can manage verification codes" ON site_verification_codes
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
-- FIXED: get_verification_code with explicit environment
...
BEGIN
  RETURN QUERY
  SELECT 
    svc.id, svc.placement, svc.code, svc.is_enabled, svc.notes,
...
-- upsert_verification_code with correct ON CONFLICT (placement, environment)
...
BEGIN
  INSERT INTO site_verification_codes (
    ...
    p_placement, p_code, p_is_enabled, p_notes, p_environment, auth.uid(), auth.uid()
  )
...
-- Seed empty verification code entries
...
-- Refund functions
-- FIXED: get_refund_request_with_download_summary is admin-only
...
-- FIXED: get_open_tickets_for_admin is admin-only
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    t.id, t.user_id, u.email::TEXT, t.subject, t.category, t.message, t.status, t.created_at
...
CREATE POLICY "Allow service role full access to promo_codes" ON promo_codes
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
CREATE POLICY "Allow service role full access to referral_events" ON referral_events
  FOR ALL TO service_role
  USING (true)
  WITH CHECK (true);
...
-- ============================================================================
-- SECTION 18: ANSWER EXAMPLE CANDIDATES (Fully Aligned with Admin Protection)
-- ============================================================================
-- This subsystem is current and used for the admin review pipeline.
-- Access model: service_role for creation, admin for review/management.
...
-- Users can only view their own submitted candidates
CREATE POLICY "Users can view own answer candidates" ON answer_example_candidates
  FOR SELECT TO authenticated
  USING (user_id = auth.uid());

-- Service role can create candidates (from edge functions)
CREATE POLICY "Service role can create candidates" ON answer_example_candidates
  FOR INSERT TO service_role
  WITH CHECK (true);

-- Admins can view all candidates for review
CREATE POLICY "Admins can view all answer candidates" ON answer_example_candidates
  FOR SELECT TO authenticated
  USING (is_admin());

-- Admins can update candidates (review workflow)
CREATE POLICY "Admins can update answer candidates" ON answer_example_candidates
  FOR UPDATE TO authenticated
  USING (is_admin())
  WITH CHECK (is_admin());

-- Service role has full access for maintenance
CREATE POLICY "Service role can manage all answer candidates" ON answer_example_candidates
  FOR ALL TO service_role
...
-- Function to create an answer candidate (service_role only)
...
-- FIXED: get_pending_answer_candidates requires admin
...
-- FIXED: update_answer_candidate_review requires admin
...
-- FIXED: get_answer_candidate_stats requires admin
...
-- ============================================================================
-- SECTION 20: SPECIFIC GRANTS (NOT blanket grants)
-- ============================================================================
...
-- Plan config
GRANT SELECT ON plan_config TO anon, authenticated;
...
-- PDFs
GRANT SELECT ON pdf_assets TO anon, authenticated;
...
-- Content - NOTE: Direct table access is admin/service_role only
GRANT SELECT ON site_announcements TO authenticated;
GRANT SELECT ON site_trust_snippets TO authenticated;
GRANT SELECT ON site_content_blocks TO authenticated;
...
GRANT SELECT ON seo_expansion_scheduler_runs TO authenticated, service_role;
GRANT SELECT ON seo_expansion_rebuild_attempts TO authenticated, service_role;
...
GRANT ALL ON seo_expansion_scheduler_runs TO service_role;
GRANT ALL ON seo_expansion_rebuild_attempts TO service_role;
...
GRANT ALL ON broadcast_messages TO service_role;
...
-- Answer candidates
GRANT SELECT ON answer_example_candidates TO authenticated;
GRANT ALL ON answer_example_candidates TO service_role;

-- Stripe webhooks
GRANT ALL ON stripe_webhook_events TO service_role;

-- ============================================================================
-- SECTION 21: FUNCTION EXECUTE GRANTS
-- ============================================================================
...
GRANT EXECUTE ON FUNCTION get_user_profile(UUID) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION soft_delete_user(UUID) TO authenticated, service_role;
...
-- Content functions - FIXED: new safer signatures without spoofable params
GRANT EXECUTE ON FUNCTION get_caller_audience_role() TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION get_active_announcements(TEXT) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_active_trust_snippets(TEXT) TO authenticated, anon;
GRANT EXECUTE ON FUNCTION get_active_content_blocks(TEXT, TEXT) TO authenticated, anon;
...
-- SEO functions - FIXED: admin-only functions restricted
...
GRANT EXECUTE ON FUNCTION get_latest_rebuild_attempt() TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION get_sitemap_sync_status_secure() TO authenticated, anon, service_role;

-- Verification code functions
GRANT EXECUTE ON FUNCTION get_verification_code(TEXT, TEXT) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION upsert_verification_code(TEXT, TEXT, BOOLEAN, TEXT, TEXT) TO authenticated, service_role;

-- Refund functions - FIXED: admin-only
...
-- Answer candidate functions - FIXED: admin-only functions restricted
GRANT EXECUTE ON FUNCTION create_answer_candidate(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION get_pending_answer_candidates(INTEGER, INTEGER) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION update_answer_candidate_review(UUID, TEXT, TEXT, BOOLEAN) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_answer_candidate_stats() TO authenticated, service_role;

-- ============================================================================
-- SECTION 22: FORCE SCHEMA RELOAD
-- ============================================================================
...
-- COMPLETION
-- ============================================================================
SELECT 'InterviewReady Fresh Install Complete!' as status;
