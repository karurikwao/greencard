...
def generate_signed_url():
    ...
    needs_premium = not pdf_asset or (pdf_asset.get('is_premium') and not pdf_asset.get('is_public_sample'))

    if needs_premium:
        has_access = db.call_function('has_premium_access', (user['id'],))
        if not has_access:
            try:
                db.call_function('record_pdf_download', (
                    user['id'], user['email'], file_key,
                    pdf_asset.get('title') if pdf_asset else file_key,
                    topic_id, category_id, 'topic_page', 'denied', None, None
                ))
            except Exception:
                pass
            return jsonify({'success': False, 'error': 'Premium subscription required', 'requiresUpgrade': True}), 403
    ...
    try:
        db.call_function('record_pdf_download', (
            user['id'], user['email'], file_key,
            pdf_asset.get('title') if pdf_asset else file_key,
            topic_id, category_id, 'topic_page', 'access_granted', None, None
        ))
    except Exception:
        pass
